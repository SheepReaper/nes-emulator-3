...
- 2026-08-24: Fresh-session shakedown exposed the missing accepted-baseline lifecycle after all three AccuracyCoin failures were resolved. `baseline update` now previews an improvement from a current complete indexed conformance run, while `--apply` atomically adopts it. Focused, stale, incomplete, infrastructure-failed, newly failing, and diagnostically changed runs are rejected; MCP discovery exposes read-only `baseline.show` and controlled `baseline.update` without expanding the three-tool surface.
